...
impl defmt::Format for SgiConfig {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "SgiConfig {{ row: {=bool:?}, china: {=bool:?}, cc: {=bool:?}, has_aes: {=bool:?}, has_des: {=bool:?}, has_sha: {=bool:?}, has_movem: {=bool:?}, has_gfmul: {=bool:?}, internal_prng: {=bool:?}, key_digest: {=bool:?}, count_size: {=bool:?}, fa: {=bool:?}, bus_width: {=bool:?}, num_datin: {=u8:?}, num_key: {=u8:?}, edc: {=bool:?}, sha_256_only: {=bool:?}, spb_support: {=bool:?}, spb_masking: {=bool:?}, sfr_sw_mask: {=bool:?} }}",
            self.row(),
            self.china(),
            self.cc(),
            self.has_aes(),
...
impl defmt::Format for SgiCtrl {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "SgiCtrl {{ start: {:?}, decrypt: {:?}, aeskeysz: {:?}, crypto_op: {:?}, insel: {:?}, outsel: {:?}, datout_res: {:?}, aes_en: {=bool:?}, gcm_en: {=bool:?}, prng_en: {=bool:?}, inkeysel: {=u8:?}, aes_no_kl: {:?}, aes_sel: {:?} }}",
            self.start(),
            self.decrypt(),
            self.aeskeysz(),
            self.crypto_op(),
...
impl core::fmt::Debug for SgiPrngSwRead {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("SgiPrngSwRead")
            .field("random", &self.random())
            .finish()
    }
}
#[cfg(feature = "defmt")]
impl defmt::Format for SgiPrngSwRead {
    fn format(&self, f: defmt::Formatter) {
