...
impl defmt::Format for Mctl {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Mctl {{ osc_div: {:?}, dis_slf_tst: {=bool:?}, rst_def: {=bool:?}, fct_fail: {=bool:?}, fct_val: {=bool:?}, ent_val: {=bool:?}, err: {=bool:?}, tstop_ok: {=bool:?}, lrun_cont: {=bool:?}, osc2_fail: {=bool:?}, prgm: {=bool:?} }}",
            self.osc_div(),
            self.dis_slf_tst(),
            self.rst_def(),
            self.fct_fail(),
...
impl defmt::Format for Osc2Ctl {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Osc2Ctl {{ trng_ent_ctl: {:?}, osc2_div: {:?}, osc2_fct_val: {=bool:?}, osc_failsafe_lmt: {:?}, osc_failsafe_test: {=bool:?} }}",
            self.trng_ent_ctl(),
            self.osc2_div(),
            self.osc2_fct_val(),
