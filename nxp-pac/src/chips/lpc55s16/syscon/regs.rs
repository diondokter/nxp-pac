...
impl defmt::Format for Ahbclkctrl1 {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Ahbclkctrl1 {{ mrt: {=bool:?}, ostimer: {=bool:?}, sct: {=bool:?}, can: {=bool:?}, utick: {=bool:?}, fc[0]: {=bool:?}, fc[1]: {=bool:?}, fc[2]: {=bool:?}, fc[3]: {=bool:?}, fc[4]: {=bool:?}, fc[5]: {=bool:?}, fc[6]: {=bool:?}, fc[7]: {=bool:?}, timer2: {=bool:?}, usb0_dev: {=bool:?}, timer0: {=bool:?}, timer1: {=bool:?} }}",
            self.mrt(),
            self.ostimer(),
            self.sct(),
            self.can(),
...
impl defmt::Format for BootLock {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "BootLock {{ lock_boot_seed: {=bool:?}, lock_hmac: {=bool:?} }}",
            self.lock_boot_seed(),
            self.lock_hmac()
        )
    }
...
impl Canclksel {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn sel(&self) -> super::vals::CanclkselSel {
        let val = (self.0 >> 0usize) & 0x07;
        super::vals::CanclkselSel::from_bits(val as u8)
    }
    #[doc = "CAN clock source select."]
    #[inline(always)]
    pub const fn set_sel(&mut self, val: super::vals::CanclkselSel) {
...
impl Clk32kclksel {
    ...
    #[must_use]
    #[inline(always)]
    pub const fn sel(&self) -> super::vals::Clk32kclkselSel {
        let val = (self.0 >> 3usize) & 0x01;
        super::vals::Clk32kclkselSel::from_bits(val as u8)
    }
    #[doc = "clock low speed source select for HS USB."]
    #[inline(always)]
    pub const fn set_sel(&mut self, val: super::vals::Clk32kclkselSel) {
